 ...
 * 6C协议基带通信速率调试
 */
public class Reader6CTrafficRateDebugActivity extends PowerManagerActivity {
	...
	private static final int[] INDEX = {0, 1, 2, 3, 4};
	private static final String[] NAME = {"快速读取模式", "密集读写模式1(M=2)", "密集读写模式2(M=4)", "自动(AutoSet)模式", "远望谷默认模式"};
	private static final String[] RATE = {"04", "05", "02", "FF", "09"};
	private static final byte PARAMETER = 0x19;
	private static final String LENGTH = "01";

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.activity_reader_debug_traffic_rate);
		setTitle(R.string.title_reader_debug_traffic_rate);

		mReaderHolder = ReaderHolder.getInstance();
		mProgressBarManager = ProgressBarManager.getInstance();

		mTrafficRateSpinner = (Spinner) findViewById(R.id.spinner_reader_debug_traffic_rate_id);
	...
	private void queryTrafficRateDebug() {
		...
		mProgressBarManager.setProgressBarMessage(mProgressBarMessageView, R.string.progress_bar_reader_debug_traffic_rate_query);
	...
	private void configTrafficRateDebug() {
		...
		int shortAnimTime = getResources().getInteger(android.R.integer.config_shortAnimTime);
		mProgressBarManager.setProgressBarMessage(mProgressBarMessageView, R.string.progress_bar_reader_debug_traffic_rate_config);
		mProgressBarManager.showProgressBar(true, mProgressBarStatusView, shortAnimTime);
	...
	private void initializeRate() {
		new Thread(new Runnable() {
			...
			public void run() {
				if (mReaderHolder.isConnected()) {
					int parameter = 0x19;
					SysQuery_800 queryTrafficRate = new SysQuery_800((byte) parameter);
	...
	private void initializeSpinner() {
		ArrayAdapter<RateEntity> rateAdapter = new ArrayAdapter<Reader6CTrafficRateDebugActivity.RateEntity>(
	...
	private class TrafficRateDebugBroadcastReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(resultCode == 0){
				...
				InvengoUtils.showToast(Reader6CTrafficRateDebugActivity.this, R.string.toast_reader_debug_traffic_rate_success);
			}else{
				//配置/查询失败
				InvengoUtils.showToast(Reader6CTrafficRateDebugActivity.this, R.string.toast_reader_debug_traffic_rate_failure);
			}
		}
	}
