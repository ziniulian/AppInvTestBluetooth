...
public class TagEPCWriteActivity extends PowerManagerActivity {
	...
	private EditText mEpcLenEditText;
	private EditText mEpcDataEditText;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_tag_6c_operation_epc_write);
		setTitle(R.string.text_tag_operation_write_epc);
		...
		mPasswordEditText = (EditText) findViewById(R.id.edit_tag_operation_write_epc_password);
		mPasswordVisibilityButton = (ToggleButton) findViewById(R.id.toggle_button_eye_visibility_write_epc);
		mEpcLenEditText = (EditText) findViewById(R.id.edit_tag_operation_write_epc_len);
		mEpcDataEditText = (EditText) findViewById(R.id.edit_tag_operation_write_epc_data);
//		mEpcDataEditText.addTextChangedListener(new HexTextWatcher(mEpcDataEditText));
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		getMenuInflater().inflate(R.menu.menu_tag_operation_epc_write, menu);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		switch (selected) {
			case R.id.menu_tag_operation_epc_write_id:
				writeEpc();
				break;
	...
	private void writeEpc() {
		...
		if(TextUtils.isEmpty(mPasswordEditText.getText().toString())){
			InvengoUtils.showToast(this, R.string.toast_tag_operation_write_epc_password);
			return;
		}
		if(TextUtils.isEmpty(mEpcLenEditText.getText().toString())){
			InvengoUtils.showToast(this, R.string.toast_tag_operation_write_epc_len);
			return;
		}
		if(TextUtils.isEmpty(mEpcDataEditText.getText().toString())){
			InvengoUtils.showToast(this, R.string.toast_tag_operation_write_epc_data);
			return;
		}

		int antenna = mSettingsCollection.getAntenna();
		byte[] accessPwd = Util.convertHexStringToByteArray(mPasswordEditText.getText().toString());
		byte[] epcData = Util.convertHexStringToByteArray(mEpcDataEditText.getText().toString());
		...
		mProgressBarManager.setProgressBarMessage(mProgressBarMessageView, R.string.progress_bar_write_epc_message);
		mProgressBarManager.showProgressBar(true, mProgressBarStatusView, shortAnimTime);
		WriteEpc msg = new WriteEpc((byte) antenna, accessPwd, epcData, tagID, tagIDType);
		OperationTask tagTask = new OperationTask(this);
		tagTask.execute(msg);
	}
	...
	private void initializeData() {
		...
		if (mSettingsCollection.getEpcChecked() == Contants.CHECKED
			...
			mEpcDataEditText.setText(matchData);
			mEpcLenEditText.setText(String.valueOf(matchData.length() / 2));
		}else if(mSettingsCollection.getTidChecked() == Contants.CHECKED){
	...
	private class TagOperationBroadcastReceiver extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent intent) {
			...
			if(resultCode == 0){
				//写入成功
				InvengoUtils.showToast(TagEPCWriteActivity.this, R.string.toast_tag_operation_write_epc_success);
			}else{
				//写入失败
				InvengoUtils.showToast(TagEPCWriteActivity.this, R.string.toast_tag_operation_write_epc_failure);
			}
		}
