	...
	private class DeviceSelectAsyncTask extends AsyncTask<String, Void, Integer>{
		...
		protected Integer doInBackground(String... params) {
			...
			if (!mDeviceType.equals(DeviceType.XC2910.toString())
					...
					&& !mDeviceType.equals(DeviceType.G6818.toString())
			) {
				if(!getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)){//for 手环式读写器
				...
				}else if(mDeviceType.equals(DeviceType.G6818.toString())) {
					deviceType = DeviceType.G6818;
					deviceName = DeviceType.G6818.toString();
					reader = new Reader("Reader1", "RS232", String.format("%s,%s", IntegrateReaderManager.getPortName(), String.valueOf(460800)));
				}
		...
		protected void onPostExecute(Integer result) {
			...
			if (!mDeviceType.equals(DeviceType.XC2910.toString())
				...
				if(result.intValue() == SUCCESS){
					...
					Intent intent = new Intent(MainActivity.this, LoginBLEActivity.class);
				...
				if(result.intValue() == SUCCESS){
					InvengoLog.i(TAG, "Enter XC2910 & XC9910 & XC2900 & XC2903 & XC2910-A & G6818 Scanner Activity.");
